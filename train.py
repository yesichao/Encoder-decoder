...
class Metrics(Callback):
    def on_epoch_end(self, epoch, logs={}):
        pred_vt_r, pred_vt_class, pred_vt = model.predict(train_sig, verbose=0)
        ...
        for j in range(pred_vt_class.shape[0]):
            pred_fin_mid = []
            for i in range(len(R_loc[j])):
                pred_fin_mid.append(np.argmax(pred_vt_class[j][R_loc[j][i]]))
            pred_fin.append(pred_fin_mid)
        Proofreading(pred_fin,true_l_class_tr,true_l_R_tr,R_loc,sig_class,
                     'D:/python/multiple_label_new/result/result_train_'+str(epoch)+'.txt')

        pred_vt_r, pred_vt_class, pred_vt = model.predict(test_sig, verbose=0)
        print('calculation...')
        pred_fin = []
        ...
        for j in range(pred_vt_class.shape[0]):
            ...
            for i in range(len(R_loc[j])):
                pred_fin_mid.append(np.argmax(pred_vt_class[j][R_loc[j][i]]))
            pred_fin.append(pred_fin_mid)
        Proofreading(pred_fin, true_l_class_te, true_l_R_te, R_loc, sig_class,
                     'D:/python/multiple_label_new/result/result_test_' + str(epoch) + '.txt')
